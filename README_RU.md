...
Установщик подключает фид пакетов Seedex и ставит `seedex-box` с командой `sdx` и `luci-app-seedex` для LuCI. Ничего не запускается, пока вы не скажете. Чтобы обойтись без LuCI, запустите установщик как `| sh -s -- --no-luci`.
